"""Official JEV HTTP client — Phase 5.

Endpoints (verified 2026-09-21):
...
Implemented on the standard library (urllib) on purpose: the brain must be able
to run on a bare Python install, so no httpx/requests requirement.
...
class OfficialJevClient(JevClient):
    ...
    def __init__(
        ...
        if not self.api_key:
            raise JevApiError(
                "no API key: set TYPESAFE_API_KEY or pass api_key=. "
